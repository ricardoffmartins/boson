...
import io.zink.josonInterface.{Joson, JosonObject}
import io.zink.josonInterface.JosonObject.{JsonArraySerializer, JsonObjectSerializer}
...
class APItests extends FunSuite{
  ...
  test("json == bson") {
    ...
    val joson: Joson = JosonObject.injector(expression, (x: Array[Byte]) => {
    ...
    val joson1: Joson = JosonObject.extractor(expression, (in: BsValue) => future.complete(in))
    joson1.go(result)
    ...
    val boson1: Boson = BosonObject.extractor(expression, (in: BsValue) => future1.complete(in))
    boson1.go(result1)
