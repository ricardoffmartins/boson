...
import io.zink.bosonInterface.BosonObject
...
class APIFuse extends FunSuite{
  ...
  test("Sequential fuse Extractor->Injector->Extractor") {
    ...
    val ext: Boson = BosonObject.extractor(expression1, (in: BsValue) => {
    ...
    val inj: Boson = BosonObject.injector(expression2, (_: Float) => newFridgeSerialCode)
