...
class InjectorParserTests extends FunSuite {
  ...
  def parseInj[T](netty: BosonImpl,f: T => T, expression: String):BsValue = {
    val parser = new TinyLanguage
    try{
      parser.parseAll(parser.program, expression) match {
        case parser.Success(r,_) =>
  ...
  test("fridgeReadings.[1 until 2] Int => Int"){
    ...
    lazy val result1: BsValue = Try(resultBoson) match {
      case Success(v) => v
      case Failure(e) => BsException.apply(e.getMessage)
    }
...
//      case BsBoolean(b) => b
  ...
  test("fridgeReadings.[1 until 2] BsonObject => BsonObject"){
    ...
    val obj3: BsonObject = new BsonObject().put("name", "João").put("age", 28).put("country", "Germany")
