...
import io.zink.boson.bson.bsonValue.{BsException, BsObject, BsValue}
...
class Interpreter[T](boson: BosonImpl, program: Program, f: Option[Function[T,T]] = None) {
  ...
  private def start(statement: List[Statement]): BsValue = {
  ...
  private def executeMoreKeys(first: Statement, list: List[Statement], dotsList: List[String]): BsValue = {
    ...
    result match {
      case Seq() => BsObject.toBson(Vector.empty[Any])
      case v => BsObject.toBson {
        (for (elem <- v) yield {
          elem match {
            case e: Array[Any] =>
              Compose.composer(e)
  ...
  private def startInjector(statement: List[Statement]): BsValue = {
  ...
  private def executeMultipleKeysInjector(statements: List[(Statement, String)]): BsValue = {
    val result:BsValue=
