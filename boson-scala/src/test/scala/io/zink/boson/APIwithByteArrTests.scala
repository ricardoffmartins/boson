...
import io.zink.bosonInterface.{BosonObject, Boson}
...
class APIwithByteArrTests extends FunSuite {
  ...
  test("extract PosV1 w/ key") {
    ...
    val boson: Boson = BosonObject.extractor(expression, (in: BsValue) => future.complete(in))
  ...
  test("extract inside loop w/ key") {
    ...
    val latch: CountDownLatch = new CountDownLatch(5)
     ...
     val boson: Boson = BosonObject.extractor(expression, (in: BsValue) => {
